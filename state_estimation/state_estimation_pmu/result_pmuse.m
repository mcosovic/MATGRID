 function [re, se] = result_pmuse(user, sys, se)
...
%  fromOrg   = string(se.fromOrg(af.on));
%  toOrg     = string(se.toOrg(af.on));
%  af.device = join([repmat({'P'}, [af.n,1]), fromOrg, toOrg],["",","]);
%  af.unit   = repmat({'(MW)'}, [af.n,1]);
% 
%  ai.device = join([repmat({'P'}, [ai.n,1]), string(se.busOrg(ai.on))], "");
%  ai.unit   = repmat({'(MW)'}, [ai.n,1]);
% 
%  va.device = join([repmat({'T'}, [va.n,1]), string(se.busOrg(va.on))], "");
%  va.unit   = repmat({'(deg)'}, [va.n,1]);
% %--------------------------------------------------------------------------
% 
% 
%-------------------------------Bus Results--------------------------------
...
% 	bad = se.bad(flo,:);
% 	num = size(bad,1);
% 	fromOrg = string(se.fromOrg(bad(:,3)));
% 	toOrg   = string(se.toOrg(bad(:,3)));
...
% 	bad = se.bad(inj,:);
% 	num = size(bad,1);
% 	device = join([repmat({'P'}, [num,1]), string(se.busOrg(bad(:,3)))], "");
% 	Device(bad(:,5)) = device;
% 
% 	vol = se.bad(:,4) == 3;
% 	bad = se.bad(vol,:);
% 	num = size(bad,1);
% 	device = join([repmat({'T'}, [num,1]), string(se.busOrg(bad(:,3)))], "");
% 	Device(bad(:,5)) = device;
% 
% 	re.bad = [table(Device, Status) array2table(Residual)];
% 
